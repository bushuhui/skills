...
### 变更前检查
14.1. 修改已有函数/类/接口前，先 grep 所有调用方，确认影响范围
14.2. 如果变更会影响其他模块，先说明影响范围，再动手

### 变更范围控制
14.3. 我的改动产生了孤立代码（未被引用的 import/变量/函数），必须清理
14.4. 不是我改动产生的既有死代码，只提建议，不删
14.5. 判断标准：每一行变更都能直接追溯到用户的某条具体请求
