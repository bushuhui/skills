...
description: |
  将 Markdown 文件转换为 Word DOCX 格式。当用户要求把 .md / Markdown 文件转成 Word 文档（.docx）时使用。触发词包括："转成 word"、"生成 docx"、"markdown 转 word"、"转 docx"、"导出为 Word"、"用这个模板转一下"。
  支持两种模式：
  - 直接转换（pandoc + LaTeX 公式支持，适合学术论文、技术文档）
  - 模板转换（python-docx 匹配参考模板样式，适合公文、技术方案、报告）
  注意：仅处理 Markdown → DOCX 转换，不处理 PDF、HTML、PPT 等其他格式。
---
...
> 以下 `SKILL_DIR` 表示本 skill 所在的实际目录路径，执行时请替换为真实路径。

```bash
# Shell 脚本（推荐）
bash "${SKILL_DIR}/md2docx.sh" <input.md> [output.docx]

# Python 脚本（含公式修正）
python3 "${SKILL_DIR}/md2docx_converter.py" <input.md> [output.docx]
```
...
```bash
python3 "${SKILL_DIR}/md2docx_template.py" <input.md> --template <template.docx> -o <output.docx>
```
