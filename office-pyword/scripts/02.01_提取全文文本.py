...
def main() -> None:
    ...
    full_text_path = TESTS_DIR / "2.1_全文纯文本.txt"
    paragraph_json_path = TESTS_DIR / "2.1_段落清单.json"

    full_text = "\n".join(str(item["text"]) for item in annotated)
    ...
    paragraph_json_path.write_text(json.dumps(paragraph_rows, ensure_ascii=False, indent=2), encoding="utf-8")

    print(f"[2.1] 输入文档: {INPUT_DOCX}")
    print(f"[2.1] 已输出全文纯文本: {full_text_path}")
    print(f"[2.1] 已输出段落清单 JSON: {paragraph_json_path}")
    print(f"[2.1] 段落数量: {len(paragraph_rows)}")
