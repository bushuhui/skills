...
**默认使用 CDP 直连 CLI**（Node.js 内置 WebSocket → Chrome 端口 9222），无需额外 proxy 进程。

所有操作都在自己创建的后台 tab 中进行，保持对用户环境的最小侵入。完成任务后关闭自己创建的 tab，保持环境整洁。
...
所有操作通过 `scripts/cdp-cli.mjs` 完成，直连 Chrome CDP 端口 9222：
...
# 执行任意 JS：可读写 DOM、提取数据、操控元素、触发状态变更、提交表单
...
# 点击（CSS 选择器）— JS el.click()，简单快速，覆盖大多数场景
...
Chrome CDP 端口 9222 持续可用，无需管理 proxy 进程。
...
**并行 CDP 操作**：每个子 Agent 在当前用户浏览器实例中，自行创建所需的后台 tab（`new`），自行操作，任务结束自行关闭（`close`）。所有子 Agent 共享一个 Chrome，通过不同 targetId 操作不同 tab，无竞态风险。
